...
public class ExpenseTrackerActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		addDialogBox.setMostFrequentedPlaces(dataSource
				.getMostFrequentedPlaces());
		if (transactionArray.isEmpty()) {
			...
			try {
				CURRENT_BALANCE = transactionArray.get(0).getCurrentBalance()
						.getFloatBalance();
	...
	public void withdrawalButtonClicked(View view) {
		if (CURRENT_BALANCE != null) {
			...
			Button addTransactionButton = addDialogBox
	...
	private void addRecord(char sign) {
		...
		if (!somePlace.toString().equals("")
				&& !somePrice.toString().equals("")) {
			Transaction transaction = new Transaction(somePlace, somePrice,
					someBalance);
	...
	private class DialogListener implements OnTouchListener {
			...
			if (e.getAction() == MotionEvent.ACTION_DOWN) {
				...
				if (v.getId() == R.id.yesButton
					...
					if (transactionArray.size() == 0) {
						...
						CURRENT_BALANCE = transactionArray.get(0)
								.getCurrentBalance().getFloatBalance();
					}
				...
				if (v.getId() == R.id.new_table_button) {
					if (newBalanceSheetDialog.getNewBalance() != null) {
						...
						Transaction invisibleTransaction = new Transaction(
								"Invisible", "0", currentBalance);
